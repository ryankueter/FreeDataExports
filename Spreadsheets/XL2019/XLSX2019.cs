        ...
        public byte[] GetBytes()
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                {
                    ...
                    var Content_Types = archive.CreateEntry("[Content_Types].xml");
                    using (var entryStream = Content_Types.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                            var doc = GetContent_Types();
                            doc.WriteTo(writer);
                        }
                    }

                    // Create xl/workbook.xml
                    var xlworkbook = archive.CreateEntry("xl/workbook.xml");
                    using (var entryStream = xlworkbook.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                    ...
                    var xlstyles = archive.CreateEntry("xl/styles.xml");
                    using (var entryStream = xlstyles.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                    ...
                    var xlsharedstrings = archive.CreateEntry("xl/sharedStrings.xml");
                    using (var entryStream = xlsharedstrings.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                            var doc = xl_sharedStrings();
                            doc.WriteTo(writer);
                        }
                    }
                    ...
                    var xlthemetheme1 = archive.CreateEntry("xl/theme/theme1.xml");
                    using (var entryStream = xlthemetheme1.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                            var doc = xl_theme_theme1();
                            doc.WriteTo(writer);
                        }
                    }

                    // Create xl/_rels/workbook.xml.rels
                    var xl_relsworkbook = archive.CreateEntry("xl/_rels/workbook.xml.rels");
                    using (var entryStream = xl_relsworkbook.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                            var doc = xl__rels_workbook();
                            doc.WriteTo(writer);
                        }
                    }

                    // Create docProps/core.xml
                    var docProps_corexml = archive.CreateEntry("docProps/core.xml");
                    using (var entryStream = docProps_corexml.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                            var doc = docProps_core();
                            doc.WriteTo(writer);
                        }
                    }

                    // Create docProps/app.xml
                    var docProps_appxml = archive.CreateEntry("docProps/app.xml");
                    using (var entryStream = docProps_appxml.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
                            var doc = docProps_app();
                            doc.WriteTo(writer);
                        }
                    }

                    // Create _rels/.rels
                    var _rels_relsxml = archive.CreateEntry("_rels/.rels");
                    using (var entryStream = _rels_relsxml.Open())
                    {
                        using (var writer = new XmlTextWriter(entryStream, System.Text.Encoding.UTF8))
                        {
                            writer.Formatting = Formatting.Indented;
